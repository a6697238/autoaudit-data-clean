...
/**
 * Created by chenguang on 2017/3/18.
 */
public class Tokenizer {
  ...
  public static String applyNGram(String content, String separator, int gram) {
    List<String> strs = apply(content);
    StringBuilder outText = new StringBuilder();
    for (int i = 0; i < strs.size(); i++) {
      ...
      for (int j = 0; j < gram; j++) {
        int index = i + j;
        if (index < strs.size()) {
          String trimStr = strs.get(index).trim();
          if (StringUtils.isNotBlank(trimStr)) {
            sb.append(trimStr);
          }
          outText.append(sb.toString()).append(separator);
        } else {
          break;
        }
      }
